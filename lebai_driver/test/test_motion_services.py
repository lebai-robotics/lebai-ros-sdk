...
def test_wait_move_releases_exclusive_gate_between_state_polls(monkeypatch):
    from lebai_driver import motion_services
    from lebai_driver.motion_services import register_motion_services
    ...
    def sleep_outside_gate(duration):
        assert gate.active is False
        clock.sleep(duration)

    monkeypatch.setattr(motion_services, 'monotonic', clock.monotonic, raising=False)
    monkeypatch.setattr(motion_services, 'sleep', sleep_outside_gate, raising=False)
    node = FakeNode()
